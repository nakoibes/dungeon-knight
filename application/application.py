...
from application.engine import Engine,GameSession
...
class Application:
    def __init__(self, name: str, config: Config):
        ...
        self._name = name
        #self._engine = Engine()
        self._menus = Menus(self._game_session)
        self._screen_resolution = (0, 0)
        if config:
            self.config_from_object(config)
        self.menu_chain = MenuHandler(self._screen_resolution, pygame.SRCALPHA, self._menus, (0, 0), ScreenHandle((0, 0)))
        self.menu_chain.connect_engine(self._game_session)
        self._game_session = GameSession(self._menus, self.menu_chain)
